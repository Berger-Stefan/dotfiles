...
local function build_and_run_current_day()
	...
	local cmd = string.format(
		"cd %s && cmake --build build && ./build/%s/%s",
		project_root,
		day_dir,
		target_name
	)
...
map("n", "<leader>r", build_and_run_current_day, { desc = "Build and run current day" })
...
map("n", "<leader>d", function() vim.diagnostic.open_float() end, { desc = "Open diagnostic float" })
